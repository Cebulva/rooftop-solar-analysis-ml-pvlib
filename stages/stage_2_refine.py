...
def render_adjust(base_resized, scaling_factor, display_h, res):
    ...
    with col_dash:
        ...
        if extracted_pts_raw:
            ...
            area = cv2.contourArea(np.array(extracted_pts_raw).astype(np.float32)) * (gsd**2)
...
def render_draw_new(base_resized, scaling_factor, display_h):
    ...
    with col_center:
        ...
        st.caption("💡 **Tip:** Click to place vertices around the roof. **Right-click** to close the shape when done.")
    ...
    with col_dash:
        ...
        if canvas_result.json_data and canvas_result.json_data["objects"]:
            ...
            if extracted_pts and len(extracted_pts) >= 3:
                lat = st.session_state.data["confirmed_lat"]
                gsd = calculate_global_gsd(lat, zoom=19)
                area = cv2.contourArea(np.array(extracted_pts).astype(np.float32)) * (gsd**2)
